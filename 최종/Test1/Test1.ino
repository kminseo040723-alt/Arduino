...
const unsigned long Steps_Per_Revolution_Close = 30000;
...
const int Move_Horizontal_Start_Speed = 100;
const int Move_Horizontal_Speed_Increment = 10;
const int Move_Horizontal_Speed_Decrement = 10;
const int Move_Horizontal_Max_Speed = 220;
const int Move_Horizontal_Min_Speed = 0;

const unsigned long Move_Horizontal_Acceleration_Interval = 500;
const unsigned long Move_Horizontal_Deceleration_Interval = 500;

const unsigned long Move_Horizontal_Accel_Duration = 5500;
const unsigned long Move_Horizontal_Decel_Duration = 8000;
...
const int Move_Horizontal_Reverse_Speed_Increment = 10;
const int Move_Horizontal_Reverse_Speed_Decrement = 10;
const unsigned long Move_Horizontal_Reverse_Acceleration_Interval = 50;
const unsigned long Move_Horizontal_Reverse_Deceleration_Interval = 50;

const unsigned long Move_Horizontal_Reverse_Accel_Duration = 500;
const unsigned long Move_Horizontal_Reverse_Decel_Duration = 700;
...
const unsigned long Climb_Duration = 40500;
const unsigned long Stop_Before_Climbing_Duration = 5000;
...
void Horizontal_Move_Accel (int direction ,int start_speed, int max_speed, int acceleration, int accel_interval) {

    if (millis() - Move_Horizontal_Change_Speed_Time  >= accel_interval) {
        Move_Horizontal_Change_Speed_Time  = millis();

        if (Move_Horizontal_Speed == 0) {
            Move_Horizontal_Speed = start_speed;
        } else {
            Move_Horizontal_Speed = min(Move_Horizontal_Speed + acceleration, max_speed);
        }
    }
    ...
    analogWrite(L_PWM[Motor_Horizontal], 0);
        ...
        analogWrite(L_PWM[Motor_Horizontal], Move_Horizontal_Speed);
    }
    
}

void Horizontal_Move_Decel (int direction, int min_speed, int deceleration, int decel_interval) {
    if (millis() - Move_Horizontal_Change_Speed_Time  >= decel_interval) {
          Move_Horizontal_Change_Speed_Time  = millis();
           Move_Horizontal_Speed = max(Move_Horizontal_Speed - deceleration, min_speed);
        }
        if (direction > 0) {
            analogWrite(R_PWM[Motor_Horizontal], Move_Horizontal_Speed);
            analogWrite(L_PWM[Motor_Horizontal], 0);
...
Signal Horizontal_Move_Update() {
    ...
    if (elapsedTime <= Move_Horizontal_Accel_Duration) {
        Horizontal_Move_Accel (1, Move_Horizontal_Start_Speed, Move_Horizontal_Max_Speed, Move_Horizontal_Speed_Increment,Move_Horizontal_Acceleration_Interval);
        return KEEP;
    } if (elapsedTime <= Move_Horizontal_Accel_Duration + Move_Horizontal_Decel_Duration) {
        Horizontal_Move_Decel (1, Move_Horizontal_Min_Speed, Move_Horizontal_Speed_Decrement, Move_Horizontal_Deceleration_Interval);
        return KEEP;
    }
...
Signal Horizontal_Move_Reverse_Update() {
    ...
    if (elapsedTime <= Move_Horizontal_Reverse_Accel_Duration) {
        Horizontal_Move_Accel (-1, Move_Horizontal_Start_Speed, Move_Horizontal_Reverse_Speed, Move_Horizontal_Reverse_Speed_Increment,Move_Horizontal_Reverse_Acceleration_Interval); 
        return KEEP;
    } if (elapsedTime <= Move_Horizontal_Reverse_Accel_Duration + Move_Horizontal_Reverse_Decel_Duration) {
         Horizontal_Move_Decel (-1, Move_Horizontal_Min_Speed, Move_Horizontal_Reverse_Speed_Decrement, Move_Horizontal_Reverse_Deceleration_Interval);
         return KEEP;
